    ...
    if (action === 'accept') {
      ...
      if (TWILIO_ACCOUNT_SID && TWILIO_AUTH_TOKEN && TWILIO_PHONE_NUMBER && booking.phone) {
        await sendSMS(
          TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN, TWILIO_PHONE_NUMBER,
          booking.phone,
          `Hi ${customerName}, unfortunately the time slot you requested for ${booking.pickup_date} is not available. Check your email for more options. — Group Ride KC`
        );
      }
      ...
      return htmlResponse(200, 'Ride Denied',
        `The ride for <strong>${customerName}</strong> on <strong>${booking.pickup_date}</strong> has been denied.<br><br>The customer has been notified via SMS and email with options to request an alternative time.`);
...
<body style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; background: #f5f5f5; padding: 20px;">
  <div style="background: #0a0a0a; color: #ffffff; border-radius: 12px; padding: 30px;">
    ...
    <p style="color: #ccc; font-size: 16px;">Hi ${booking.customer_name || 'there'},</p>
    <p style="color: #ccc;">We're sorry, but the time slot you requested for <strong>${booking.pickup_date}</strong> at <strong>${booking.pickup_time}</strong> is not currently available.</p>
...
<body style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; background: #f5f5f5; padding: 20px;">
  <div style="background: #0a0a0a; color: #ffffff; border-radius: 12px; padding: 30px;">
    ...
    <table style="width: 100%; border-collapse: collapse; margin: 20px 0;">
      ...
      <tr><td style="padding: 8px 0; color: #aaa;">Original Date</td><td style="padding: 8px 0; color: #fff;">${booking.pickup_date}</td></tr>
      <tr><td style="padding: 8px 0; color: #aaa;">Original Time</td><td style="padding: 8px 0; color: #fff;">${booking.pickup_time}</td></tr>
...
<body>
  <div class="page">
    <div class="card">
      <h1>Ride Accepted ✓</h1>
      <p>The ride for <strong>${customerName}</strong> on <strong>${booking.pickup_date}</strong> has been accepted.</p>
